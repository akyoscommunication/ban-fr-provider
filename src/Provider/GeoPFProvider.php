...
use Akyos\BANFrProvider\Model\GeoPFAddress;
...
class GeoPFProvider extends AbstractHttpProvider
{
    const GEOPF_TYPE = 'type';
    private const GEOCODE_ENDPOINT_URL = 'https://data.geopf.fr/geocodage/search?q=%s&type=%s';
    private const REVERSE_ENDPOINT_URL = 'https://data.geopf.fr/geocodage/reverse?lat=%F&lon=%F';
    ...
    private function parseResponse(ResponseInterface $response): Collection
    {
        ...
        if (isset($json['features']) && is_array($json['features'])) {
            ...
            foreach ($json['features'] as $feature) {
                ...
                /** @var GeoPFAddress $address */
                $address = $builder->build(GeoPFAddress::class);

//  "score" => 0.86983818181818
//  "id" => "21038_0720_00026"
//  "banId" => "ea63df2d-4ec5-4ae6-b2e0-2253e1ef1a7b"
//  "name" => "26 Rue Colonel Denfert"
//  "postcode" => "21130"
//  "citycode" => "21038"
//  "x" => 880671.34
//  "y" => 6679634
//  "city" => "Auxonne"
//  "context" => "21, Côte-d'Or, Bourgogne-Franche-Comté"
//  "type" => "housenumber"
//  "importance" => 0.56822
//  "street" => "Rue Colonel Denfert"
//  "_type" => "address"
                ...
                if (isset($properties['banId']) && $banId = $properties['banId']) {
